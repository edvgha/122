...
while True:
    ...
    #approximate with ∂2f(x)/∂x2 so we take only diagonal of hessian
    H = cvx.diag(cvx.diag(hess))
    v = numpy.dot(-numpy.linalg.inv(H.value), grad)
    #evalue lambda^2
...
axes[2].set_title('Newton descent app2')
#----------------------------------------#
fig.tight_layout()
plt.show()
#----------------------------------------#
