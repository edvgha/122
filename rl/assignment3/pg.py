...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    if self.discrete:
      self.action_placeholder = tf.placeholder(tf.float32, shape=(None, ), name="discrete_action")
    else:
      self.action_placeholder = tf.placeholder(tf.float32, shape=(None, self.action_dim), name="cont_action")
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      action_logits = build_mlp(self.observation_placeholder, output_size=self.action_dim, scope=scope, n_layers=self.config.n_layers, size=self.config.layer_size, output_activation=self.config.activation)
      ...
      action_means = action_means = build_mlp(self.observation_placeholder, output_size=self.action_dim, scope=scope, n_layers=self.config.n_layers, size=self.config.layer_size, output_activation=self.config.activation)
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = build_mlp(self.observation_placeholder, output_size=1, scope=scope, n_layers=self.config.n_layers, size=self.config.layers_size)
    self.baseline_target_placeholder = tf.placeholders(tf.float32, shpae=(None, ))
