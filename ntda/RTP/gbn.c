...
int checksum(int ack, int seqnum, char message[20])
{
    uint32_t checksum = (uint32_t) seqnum + (uint32_t) ack;
    union converter con;
...
{
    uint32_t checksum = (uint32_t)packet.seqnum + (uint32_t)packet.acknum;
    union converter con;
    for (int i = 0; i < 20; i += 2) {
        con.c[0] = packet.payload[i];
        con.c[1] = packet.payload[i + 1];
        checksum += con.d;
    }
    checksum &= 0xffff;
    uint16_t res = (uint16_t)checksum;
...
struct pkt make_pkt(int ack, int seqnum, char message[20], int checksum)
{
    struct pkt packet;
    packet.acknum = ack;
...
void B_input(struct pkt packet)
{
    if (0 == is_corrupted(packet) &&
        ...
        b_sndpkt = make_pkt(1, b_expectedseqnum, message, checksum(1, b_expectedseqnum, message));
        tolayer3(B, b_sndpkt);
        ++b_expectedseqnum;
        return ;
    }
...
void B_init()
{
    ...
    b_sndpkt = make_pkt(1, 0, message, checksum(1, 0, message));
    tolayer3(B, b_sndpkt);
}
