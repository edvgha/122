...
void belongs_to_range(const std::unique_ptr<BSTNode<int, char>>& n, std::pair<int, int> range, std::vector<int>& res)
{
    if (!n) return;
    if (n->data1 >= range.first && n->data1 <= range.second) {
        res.push_back(n->data1);
        belongs_to_range(n->right, range, res);
        belongs_to_range(n->left, range, res);
    } else if (n->data1 > range.second) {
        belongs_to_range(n->left, range, res);
    } else if (n->data1 < range.first) {
        belongs_to_range(n->right, range, res);
    }
}
...
int main()
{
	...
	build();
	//print_inorder(root);
	belongs_to_range(root, {5, 41}, res);
    for (auto item : res) 
