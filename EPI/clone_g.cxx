...
struct vertex
{
    ...
    vertex* clone() { return new vertex(data); }
};
...
vertex* clone_graph(vertex* v)
{
    ...
    while (!q.empty())
    {
        ...
        for (size_t i = 0; i < u -> edges.size(); ++i) 
        {
            if (m.find((u -> edges)[i]) == m.end())
            {
                vertex* u_clone = (u -> edges)[i] -> clone();
                m[(u -> edges)[i]] = u_clone;
                q.push((u -> edges)[i]);
            }
            m[u] -> edges.push_back(m[(u -> edges)[i]]);
        }
    }
    return m[v];
}
