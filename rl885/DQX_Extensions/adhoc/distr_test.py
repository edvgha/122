...
if __name__ == "__main__":
    ...
    proj_hist = lib.dqn_extra.distr_projection(src_hist, np.array([2], dtype=np.float32), np.array([False]), gamma=0.9)
    ...
    src_hist = hist[0]
    proj_hist = lib.dqn_extra.distr_projection(np.array([src_hist]), np.array([2], dtype=np.float32), np.array([False]), gamma=0.9)
    save_distr(hist[0], proj_hist[0], "normal-r=2")

    # normal distribution, but done episode
    proj_hist = lib.dqn_extra.distr_projection(np.array([src_hist]), np.array([2], dtype=np.float32), np.array([True]), gamma=0.9)
    save_distr(hist[0], proj_hist[0], "normal-done-r=2")

    # clipping for out-of-range distribution
    proj_dist = lib.dqn_extra.distr_projection(np.array([src_hist]), np.array([10], dtype=np.float32), np.array([False]), gamma=0.9)
